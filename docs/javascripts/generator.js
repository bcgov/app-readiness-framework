...
(function () {
  ...
  var CATALOG = [
    ...
    I("design", "Document integrations, interfaces and external dependencies",
      ...
      ["M","M","S"]),
    I("design", "Meet WCAG 2.2 AA accessibility for any user interface (Accessible BC Act)",
      "Accessibility is a legal requirement for public-facing government services and expected for internal ones; retrofitting it later is costly.",
      "Accessibility conformance assessment against WCAG 2.2 AA.",
      ["M","M","S"]),
    I("design", "Publish a versioned API contract (OpenAPI / AsyncAPI) with a deprecation policy",
      "Consumers depend on a stable, documented contract; unversioned APIs break integrations without warning.",
      "OpenAPI/AsyncAPI spec in the repo; versioning + deprecation policy documented.",
      ["M","M","S"]),
    ...
    I("build", "Add DAST / OWASP scanning for the web front end",
      ...
      ["M","S","Y"]),
    I("build", "Define the test strategy — unit / integration / contract tests, UAT, and managed test data",
      "Coverage alone isn't a strategy; contract tests protect integrations and UAT confirms it meets the need.",
      "Test strategy doc; contract tests for integrations; test-data approach.",
      ["M","M","S"]),
    I("build", "Run automated accessibility testing in the pipeline (axe / pa11y) for web UIs",
      "Manual accessibility checks don't scale; automated tests catch regressions on every build.",
      "Accessibility test stage in the pipeline with results.",
      ["M","S","Y"]),
    I("build", "Check open-source licence compliance (no prohibited / copyleft licences; attributions met)",
      "The SBOM inventories dependencies; licence review confirms we're allowed to use and distribute them.",
      "Licence scan report; flagged licences reviewed and approved.",
      ["M","M","S"]),
    I("build", "Keep runtimes and frameworks in vendor support — no end-of-life versions; defined patching cadence",
      "End-of-life runtimes stop getting security fixes; technical currency is what keeps an app supportable over time.",
      "Currency check (no EOL versions) + documented patching cadence.",
      ["M","M","S"]),
    I("build", "Externalise configuration and manage infrastructure as code (no manual console changes)",
      "Config baked into images or changed by hand causes drift and un-reproducible environments.",
      "Config externalised per environment; IaC in the repo; no manual drift.",
      ["M","M","S"]),
    ...
    I("resilience", "Verify a zero-downtime rolling update",
      ...
      ["M","M","S"]),
    I("resilience", "Use progressive delivery — blue/green or canary with feature flags to limit blast radius",
      "A bad release should reach few users and roll back instantly, not take everyone down at once.",
      "Blue/green or canary config + a feature-flag mechanism.",
      ["M","S","Y"]),
    ...
    I("security", "Understand and own the vulnerability-management SLAs",
      ...
      ["M","M","S"]),
    I("security", "Enforce API management — gateway, rate limiting / throttling, and auth on every endpoint",
      "Ungoverned endpoints get abused or overloaded; a gateway centralises auth, rate limits, and monitoring.",
      "API gateway config; rate limiting/throttling; auth on all endpoints.",
      ["M","S","Y"]),
    I("security", "Confirm data residency and privacy-by-design (in-Canada / approved region; minimise & mask personal data)",
      "FOIPPA and government policy constrain where data can live and how personal information is handled.",
      "Data-residency confirmation; privacy-by-design notes tied to the PIA.",
      ["M","M","S"]),
    ...
    I("operability", "Provide a defect 'tombstone' list at go-live",
      ...
      ["M","M","S"]),
    I("operability", "Document a decommission / end-of-life plan (data disposition, dependency and cert/DNS teardown)",
      "Apps with no retirement plan become zombie risk — orphaned data, expiring certs, and forgotten dependencies.",
      "Decommission plan: data disposition, teardown steps, and a named owner.",
      ["M","S","S"]),
